...
function logging.new(append, startLevel)
    ...
    for i = 1, MAX_LEVELS do
        ...
        if logger[level:lower()] then
            return nil,
                "'" .. level ..
                "' is not a proper level name since there is already a property '" .. level:lower() .. "'"
        end
...
local getDebugInfoLine = debug and "local info = debug.getinfo(%d)" or
"local info = { short_src = '?', currentline = -1 }"
...
function logging.defaultLogPatterns(patt)
    if patt then
        ...
        for _, level in ipairs(LEVELS) do
            if level ~= "OFF" then
                assert(type(patt[level]) == "string",
                    "the patterns contains a '%s' value (instead of a string) for level '%s'", type(patt[level]), level)
            end
        end
...
function logging.buildLogPatterns(patterns, default)
    ...
    assert(type(default) == "string" or type(default) == "nil",
        "expected default logPattern (2nd argument) to be a string or nil, got: %s", tostring(default))
    assert(type(patterns) == "table", "expected patterns (1st argument) to be a table or nil, got: %s",
        tostring(patterns))
    local target = {}
    for _, level in ipairs(LEVELS) do
        if level ~= "OFF" then
